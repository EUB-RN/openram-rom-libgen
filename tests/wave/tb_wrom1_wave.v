// ---------------------------------------------------------------------------
// tb_wrom1_wave -- WAVEFORM testbench for wrom1, generated by gen_wave_tb.py
//
// Regenerate:  python3 scripts/rom_char/gen_wave_tb.py wrom1
//
...
// so the period is roughly 60.0 ns -- this macro is SLOW, do not expect
...
module tb_wrom1_wave;
  ...
  //     gen_wave_tb.py wrom1 --setup-margin 0.6
  //     gen_wave_tb.py wrom1 --hold-margin 0.5
  //     gen_wave_tb.py wrom1 --pre-margin 0.8 --eval-margin 0.9
  ...
  // hold_rising is the whole access (43.3153 ns), and
  ...
  localparam real MPW_HIGH = 44.2403;    // min_pulse_width, rise
  localparam real ACCESS   = 43.3153;    // clk0 rise -> dout0 valid
  localparam real SETUP    = 0.0480;    // setup_rising, addr0
  localparam real HOLD     = 38.3781;    // hold_rising, addr0
  localparam real T_FALL   = 3.4192;    // falling_edge arc on dout0
  ...
  integer i;
  initial begin
    $dumpfile("tb_wrom1_wave.vcd");
    $dumpvars(0, tb_wrom1_wave);

    $display("=== wrom1 waveform run: %0d reads from address %0d ===",
             N_READS, START_ADDR);
    ...
    for (i = 0; i < N_READS; i = i + 1) begin
      ...
      // is 38.3781 ns rather than the full access window.
    ...
    if (errors == 0)
      $display("=== done: %0d reads, every one matched wrom1.bin ===", N_READS);
    else
