...
// Word per Row: 8
// Data Type: bin
// Data File: rom_configs/wrom3.bin
//
...
// ---------------------------------------------------------------------------
// wrom3 -- REAL BEHAVIOURAL MODEL -- gen_macro_behavioral_v.py
//
...
//     python3 scripts/rom_char/gen_macro_behavioral_v.py wrom3
//
...
// BOTH ARE FALSE. This macro's extracted cell inventory (wrom3/*.ext) has no
...
//
// REAL BEHAVIOUR (values MEASURED with ngspice, from wrom3's own .lib)
// ---------------------------------------------------------------------------
...
//              bitline through 77 series NMOS (worst column 10).
...
// TIMING   : SS_1p6V_100C corner (the worst one). Source: wrom3_SS_1p6V_100C.lib
//
// SIMULATION ONLY. Not synthesizable; the ASIC flow reads wrom3_bbox.v.
//
...
// a .v name: set_property file_type SystemVerilog [get_files wrom3.v].
// ---------------------------------------------------------------------------
`timescale 1ns / 1ps
...
`endif
  ...
  parameter WIDTH     = 32;
  parameter INIT_FILE = "rom_configs/wrom3.bin";

  // MEASURED values -- wrom3_SS_1p6V_100C.lib
  parameter real ACCESS_NS = 39.8293;   // clk0 rising -> dout0 valid
  parameter real T_PRE_NS  = 16.5034;   // minimum clk0 low phase
  ...
  parameter real MPW_HIGH_NS = 40.7543;   // minimum clk0 high phase
  ...
  parameter real T_FALL_NS   = 2.8806;   // clk0 falling -> dout0 back at VDD
