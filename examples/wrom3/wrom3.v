...
// yanlis olur.
// ---------------------------------------------------------------------------
// wrom3 -- GERCEK DAVRANIS MODELI -- gen_macro_behavioral_v.py
//
...
//     python3 asic/scripts/rom_char/gen_macro_behavioral_v.py wrom3
//
...
// IKISI DE YANLIS. Bu makronun cikarilmis hucre envanterinde (wrom3/*.ext)
...
//
// GERCEK DAVRANIS (degerler wrom3'in kendi .lib'inden, ngspice ile OLCULDU)
// ---------------------------------------------------------------------------
//   clk0 = 0 : ON-SARJ. Bitline'lar VDD'ye cekilir -> dout0 = tumu 1.
//              Bu faz en az T_PRE_NS surmeli.
//   clk0 = 1 : DEGERLENDIRME. Secili satirin "0" bitleri bitline'i 77
//              seri NMOS uzerinden bosaltir (en kotu kolon 10).
...
//              wrom3_rom_control_logic.ext merge zincirinden dogrulandi.)
//
...
// ZAMANLAMA: SS_1p6V_100C kosesi (en kotu). Kaynak: wrom3_SS_1p6V_100C.lib
//
// SADECE SIMULASYON. Sentezlenmez; ASIC akisi wrom3_bbox.v (blackbox) okur.
// ---------------------------------------------------------------------------
`timescale 1ns / 1ps
...
`endif
  ...
  // OLCULEN degerler -- wrom3_SS_1p6V_100C.lib
  parameter real ACCESS_NS = 41.2913;   // clk0 yukselen -> dout0 gecerli
  parameter real T_PRE_NS  = 15.3709;   // clk0 dusuk fazi en az bu kadar
