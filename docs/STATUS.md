...
deck with the waveform kept -- the measured run asks only for `.measure`
results, so the samples are thrown away -- and the generator replays them PWL
...
### `run_waveform_capture.sh` is gone, and no script replaces it

It re-ran a measured deck with the waveform kept and let ngspice's own
`hardcopy` write `05`/`05b`/`06`/`07` as SVG. No plotting tool was involved
and the pictures were honest, which is not the point: **a figure in this
repository is the independent check that the committed numbers describe the
circuit somebody actually simulated.** Evidence generated by the same
machinery it is meant to check -- a script in this flow, or an AI editing that
flow -- is not independent of it, whatever its output looks like. So the
figures are captured by a person at ngspice's plot window, and the script that
automated it was removed rather than kept as a convenience.

What this does NOT cover is the back-end deck's `char/wave/bl_<corner>.txt`.
That is a waveform a script keeps, but it is the stimulus of a measurement
rather than a picture of a result: the column deck's discharge, replayed into
the back end, git-ignored, rebuilt from the deck it came from, and it produces
a `.lib` number that every other check in the flow can be pointed at. It is
named here so the distinction is deliberate rather than an oversight.

Left behind: `docs/img/05-col-discharge.svg` and `06-front-end.svg`, which
that script drew. They are still accurate for what they show, but they are
...
* **`docs/img/07-backend-loads.svg` is missing and `docs/measurements.md`
  ...
  load against the bitline driving it. It would also be the first picture of
  the back end driven by the replayed edge. `docs/macro.md` links
  `img/04-column-strip.png`, which has never existed and is marked *(missing)*
  in `docs/img/README.md`.
